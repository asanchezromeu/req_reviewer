...
try:
    from backend.deterministic_review import Requirement, parse_requirements, review_requirement, review_requirements
except ImportError:
    from deterministic_review import Requirement, parse_requirements, review_requirement, review_requirements
...
class DeterministicReviewTests(unittest.TestCase):
    def test_csv_sample_parses_traceability(self):
        requirements = parse_requirements(
            ...
            (ROOT / "legacy" / "streamlit-prototype" / "samples" / "requirements_sample.csv").read_bytes(),
        )
